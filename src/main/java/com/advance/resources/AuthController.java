...
public class AuthController {
	...
	@PostMapping("/login")
	public ResponseEntity<HttpResponse> loginUser(@RequestBody LoginForm form) {
		Authentication authenticate = authManager.authenticate(new UsernamePasswordAuthenticationToken(form.getEmail(), form.getPassword()));
		UserDTO dto = ((UserPrincipal) authenticate.getPrincipal()).getUser();
		return ResponseEntity.ok()
				.body(HttpResponse.builder().timeStamp(LocalDateTime.now().toString()).message("Successfully logged in!")
						.status(HttpStatus.OK).statusCode(HttpStatus.OK.value()).data(Map.of("user", dto))
						.build());
	}
}
